    ...
    strategies = [
        {
            ...
            "win_rates": {"爱乐达": "83.3%", "ST炼石": "69.6%", "高澜股份": "50.0%"},
            "entry": "RSI或布林带位置低于阈值，且当日上涨收阳。爱乐达用自适应阈值+MA20趋势过滤；ST炼石用固定阈值(RSI<33/BB<0.5)无趋势过滤；高澜股份用自适应阈值无趋势过滤",
            "exit": "RSI > 自适应卖出阈值(约50-70) 或 布林带位置 > 0.75",
            "timing": "T+1开盘买入，T+6开盘卖出",
            "description": "利用RSI超卖和布林带下轨支撑，捕捉短期反弹机会。各股票采用不同参数：爱乐达用自适应参数+MA20趋势过滤(胜率最高83.3%)；ST炼石用固定参数无过滤(回测胜率更优)；高澜股份用自适应参数无趋势过滤(适合逆势抄底)。",
        },
        {
            "id": 2,
            "name": "MA支撑+KDJ超卖",
        ...
        },
        {
            ...
            "entry": "RSI <= 阈值(扬农化工固定35，其他自适应约25-40) 且连续下跌 >= 2天",
            "exit": "RSI > 50 或连续上涨2天",
            "timing": "部分股票T+0尾盘买入(高澜/裕同/扬农/华测/川润)，其余T+1开盘买入；对应T+5尾盘或T+6开盘卖出",
            "description": "捕捉连续下跌后的超跌反弹机会，RSI超卖确认下跌动能衰竭。扬农化工使用固定RSI<=35阈值(回测胜率更优)，其他股票采用自适应参数。不同股票根据回测结果采用不同的买卖时机。",
        },
        {
            "id": 5,
            "name": "动量策略",
            "color": "#E74C3C",
            "stocks": ["川润股份", "英维克", "爱乐达"],
            "win_rates": {"川润股份": "59.3%", "英维克": "53.1%", "爱乐达": "64.7%"},
