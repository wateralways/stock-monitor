...
def generate_strategy_doc():
    strategies = [
        ...
        },
        {
            ...
            "color": "#D4A017",
            "stocks": ["川润股份"],
            "win_rates": {"川润股份": "54.3%(T+7)"},
            "entry": "成交量/5日均量<0.8(缩量) 且 当日涨幅>1% 且 收盘价>MA20",
            "exit": "持仓7个交易日 或 止损-10%",
            "timing": "T+1开盘买入(涨停不买)，T+7开盘卖出 或 盘中触及-10%次日开盘止损",
            "description": "川润股份专用策略。利用缩量上涨识别主力控盘信号——成交量萎缩但价格上涨说明抛压已尽、筹码锁定。MA20过滤确保在上升趋势中操作。胜率54.3%但盈亏比1.90:1(赢均+13%/亏均-6.9%)，靠盈亏比而非胜率盈利。2025年以来累计+138.6%。涨停命中率56%(MA20以上,7天内)。",
        },
    ]
