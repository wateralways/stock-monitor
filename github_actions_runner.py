    ...
    strategies = [
        {
            ...
            "win_rates": {"爱乐达": "80.0%", "ST炼石": "79.0%", "高澜股份": "44.0%"},
            "entry": "自适应RSI阈值(根据波动率动态计算，约20-45)或布林带位置低于自适应阈值(约0.1-0.6)，且当日上涨收阳。非ST股需MA20趋势未明显下行(5日斜率>-1%)",
            "exit": "RSI > 自适应卖出阈值(约50-70) 或 布林带位置 > 0.75",
            "timing": "T+1开盘买入，T+6开盘卖出",
            "description": "利用RSI超卖和布林带下轨支撑，捕捉短期反弹机会。引入自适应参数系统，根据近期波动率(20日/60日波动率比值)动态调整买卖阈值，高波动时条件更严格。非ST股增加MA20趋势过滤，避免在下跌趋势中抄底。",
        },
        {
            "id": 2,
            "name": "MA支撑+KDJ超卖",
        ...
        },
        {
            ...
            "color": "#E67E22",
            "stocks": ["高澜股份", "英维克"],
            "win_rates": {"高澜股份": "61.54%", "英维克": "无独立回测数据"},
        ...
        },
        {
            ...
            "entry": "RSI <= 自适应阈值(根据波动率计算，约25-40) 且连续下跌 >= 2天",
            "exit": "RSI > 50 或连续上涨2天",
            "timing": "部分股票T+0尾盘买入(高澜/裕同/扬农/华测/川润/英维克)，其余T+1开盘买入；对应T+5尾盘或T+6开盘卖出",
            "description": "捕捉连续下跌后的超跌反弹机会，RSI超卖确认下跌动能衰竭。RSI阈值采用自适应参数，波动率高时要求更极端的超卖。不同股票根据回测结果采用不同的买卖时机。",
        },
        {
            "id": 5,
            "name": "动量策略",
            "color": "#E74C3C",
            "stocks": ["川润股份", "英维克", "爱乐达"],
            "win_rates": {"川润股份": "67.0%", "英维克": "62.0%", "爱乐达": "62.0%"},
            ...
            "description": "综合动量指标，捕捉上涨趋势中的加速信号。引入自适应量比阈值和MA20趋势过滤(单动量信号在下跌趋势中不触发)。每只股票根据历史回测禁用容易产生假信号的子策略，针对性优化。",
        },
    ]
