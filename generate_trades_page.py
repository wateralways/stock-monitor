...
COMBOS = [
    ...
    ("底部抬高+温和放量(拓日)", "#2C7873", check_s9, [
        ("002218.SZ", "拓日新能"),
    ], (1, 4)),  # T+1/T+4 (76.5%胜率, 平均+5.08%, 方案A优化)
    ("底部抬高+温和放量(ST炼石)", "#2C7873", check_s9, [
        ("000697.SZ", "ST炼石"),
    ], (1, 4)),  # T+1/T+4 (83.3%胜率, 平均+4.22%, 方案A优化)
    ("底部抬高+温和放量(华夏航空)", "#2C7873", check_s9, [
        ("002928.SZ", "华夏航空"),
    ], (1, 6)),  # T+1/T+6 (85.7%胜率, 平均+3.97%)
    ("底部抬高+温和放量(英维克)", "#2C7873", check_s9, [
        ("002837.SZ", "英维克"),
    ], (1, 8)),  # T+1/T+8 (80.0%胜率, 平均+2.52%)
...
def main():
    ...
    # 始终从2024-09-01开始拉取，保证新增combo在2025-01-01~02的信号也能被扫描到
    # (idx=60 需要至少60个交易日预热，即2024-11起数据才够)
    data_start = "20240901"
    print(f"\n获取数据 (起始: {data_start})...")
    cache = {}
